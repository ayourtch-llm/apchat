use apchat::chat::ReadlineInstance;
...
fn test_lifecycle_transitions() {
    ...
    for cycle in 0..5 {
        ...
        // Add history entries
        crate::chat::ReadlineInstance::add_history(&format!("cycle_{}_entry_1", cycle)).unwrap();
        crate::chat::ReadlineInstance::add_history(&format!("cycle_{}_entry_2", cycle)).unwrap();
...
fn test_history_persistence_across_operations() {
    ...
    // Add initial entries
    crate::chat::ReadlineInstance::add_history("persist_test_1").unwrap();
    crate::chat::ReadlineInstance::add_history("persist_test_2").unwrap();
    
    // Save history
    crate::chat::ReadlineInstance::save_history().unwrap();
    
    // Add more entries
    crate::chat::ReadlineInstance::add_history("persist_test_3").unwrap();
...
fn test_cleanup_does_not_prevent_reuse() {
    ...
    crate::chat::ReadlineInstance::add_history("before_cleanup").unwrap();
    crate::chat::ReadlineInstance::cleanup().unwrap();
    
    // Verify instance is still usable after cleanup
    crate::chat::ReadlineInstance::add_history("after_cleanup").unwrap();
...
fn test_rapid_cleanup_saves() {
    ...
    for i in 0..10 {
        crate::chat::ReadlineInstance::add_history(&format!("rapid_save_{}", i)).unwrap();
        crate::chat::ReadlineInstance::save_history().unwrap();
    }
    ...
    crate::chat::ReadlineInstance::add_history("after_rapid_ops").unwrap();
...
fn test_history_boundary_conditions() {
    ...
    crate::chat::ReadlineInstance::add_history("").unwrap();
    
    // Test with very long strings
    let long_string = "a".repeat(10000);
    crate::chat::ReadlineInstance::add_history(&long_string).unwrap();
    
    // Test with special characters
    crate::chat::ReadlineInstance::add_history("!@#$%^&*()[]{}|\\<>?").unwrap();
    
    // Test with unicode
    crate::chat::ReadlineInstance::add_history("Hello 世界 🌍").unwrap();
