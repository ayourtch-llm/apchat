...
pub struct Session {
    ...
    pub apchat: Arc<tokio::sync::Mutex<APChat>>,
...
impl Session {
    pub fn new(
        ...
        Self {
            ...
            apchat: Arc::new(tokio::sync::Mutex::new(apchat)),
    ...
    pub async fn get_info(&self) -> SessionInfo {
        let apchat = self.apchat.lock().await;
        let clients = self.clients.read().await;
        ...
        SessionInfo {
            ...
            message_count: apchat.messages.len(),
            current_model: apchat.current_model.display_name().to_string(),
...
impl SessionManager {
    ...
    pub async fn create_session(&self, config: SessionConfig) -> Result<SessionId> {
        ...
        let model = apchat_models::ModelColor::from_str(model_str)
            .map_err(|e| anyhow::anyhow!("Invalid model '{}': {}", model_str, e))?;

        // Create APChat instance
        ...
        apchat.non_interactive = true; // Web sessions should not prompt for input
            ...
            for session_id in session_ids {
                match persistence.load_session(&session_id) {
                    Ok(persistent_session) => {
                        // Create APChat instance from saved state
                        let mut apchat = APChat::new_with_config(
                            self.client_config.clone(),
                            self.work_dir.clone(),
                        ...
                        // Restore state
                        apchat.messages = persistent_session.chat_state.messages;
                        apchat.current_model = persistent_session.chat_state.current_model;
                        apchat.total_tokens_used = persistent_session.chat_state.total_tokens_used;
                        apchat.non_interactive = true;
