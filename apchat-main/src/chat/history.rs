...
// ============================================================================
// History Validation and Recovery Functions
// ============================================================================
...
pub fn validate_history(messages: &[Message]) -> Result<()> {
    if messages.is_empty() {
        return Ok(()); // Empty history is valid
    }
    ...
    for (i, message) in messages.iter().enumerate() {
        ...
        if !valid_roles.contains(&message.role.as_str()) {
            return Err(anyhow::anyhow!(
                "Invalid role '{}' at message {}. Must be one of: system, user, assistant, tool",
                message.role, i
            ));
        }

        // Validate content for non-tool messages
        if message.role != "tool" && message.content.is_empty() {
            return Err(anyhow::anyhow!(
                "Empty content at message {} with role '{}'",
                i, message.role
            ));
        }
        ...
        if message.role == "tool" && message.tool_call_id.is_none() {
            return Err(anyhow::anyhow!(
                "Tool message at index {} is missing tool_call_id",
                i
            ));
        }

        // Validate tool_calls for assistant messages
        if message.role == "assistant" {
            if let Some(tool_calls) = &message.tool_calls {
                if tool_calls.is_empty() {
                    return Err(anyhow::anyhow!(
                        "Assistant message at index {} has empty tool_calls",
                        i
                    ));
                }
            }
    ...
    for i in 1..messages.len() {
        ...
        if prev_role == "system" || curr_role == "system" {
            continue;
        }

        // After user should come assistant or tool
        if prev_role == "user" && curr_role != "assistant" && curr_role != "tool" {
            return Err(anyhow::anyhow!(
                "Invalid role sequence at index {}: '{}' following '{}'",
                i, curr_role, prev_role
            ));
        }

        // After assistant should come user or tool
        if prev_role == "assistant" && curr_role != "user" && curr_role != "tool" {
            return Err(anyhow::anyhow!(
                "Invalid role sequence at index {}: '{}' following '{}'",
                i, curr_role, prev_role
            ));
        ...
        // After tool should come assistant
        if prev_role == "tool" && curr_role != "assistant" {
            return Err(anyhow::anyhow!(
                "Invalid role sequence at index {}: '{}' following '{}'",
                i, curr_role, prev_role
            ));
...
pub fn fix_interrupted_history(messages: &[Message]) -> (Vec<Message>, bool) {
    let mut fixed_messages = messages.to_vec();
    let mut changes_made = false;

    if fixed_messages.is_empty() {
        return (fixed_messages, changes_made);
    }

    // Check for interrupted sequences and fix them
    for i in 1..fixed_messages.len() {
        let prev_role = &fixed_messages[i - 1].role;
        let curr_role = &fixed_messages[i].role;

        // Fix: assistant -> assistant (missing user message)
        if prev_role == "assistant" && curr_role == "assistant" {
            let user_recovery_msg = Message {
                role: "user".to_string(),
                content: "[RECOVERY: Continuing conversation]".to_string(),
                tool_calls: None,
                tool_call_id: None,
                name: None,
                reasoning: None,
            };
            fixed_messages.insert(i, user_recovery_msg);
            changes_made = true;
            // Skip the next comparison since we inserted a message
            continue;
        }

        // Fix: tool -> tool (missing assistant message)
        if prev_role == "tool" && curr_role == "tool" {
            let assistant_recovery_msg = Message {
                role: "assistant".to_string(),
                content: "[RECOVERY: Continuing after tool execution]".to_string(),
                tool_calls: None,
                tool_call_id: None,
                name: None,
                reasoning: None,
            };
            fixed_messages.insert(i, assistant_recovery_msg);
            changes_made = true;
            continue;
        }

        // Fix: user -> tool (missing assistant message with tool call)
        if prev_role == "user" && curr_role == "tool" {
            let assistant_recovery_msg = Message {
                role: "assistant".to_string(),
                content: "[RECOVERY: Tool execution initiated]".to_string(),
                tool_calls: None,
                tool_call_id: None,
                name: None,
                reasoning: None,
...
pub fn insert_bogus_message(
    ...
    let bogus_message = Message {
        role: role.to_string(),
        content: format!("[BOGUS: {}]", content),
        tool_calls: None,
        tool_call_id: None,
        name: None,
        reasoning: None,
    ...
    if position <= new_messages.len() {
        new_messages.insert(position, bogus_message);
    } else {
        new_messages.push(bogus_message);
    }
