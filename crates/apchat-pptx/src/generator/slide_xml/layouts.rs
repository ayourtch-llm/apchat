...
use super::common::{SLIDE_HEADER, SLIDE_FOOTER, generate_title_shape, create_slide_header_with_bg};
...
pub fn create_blank_slide(content: &SlideContent, chart_rids: &[String]) -> String {
    let header = if let Some(bg_color) = &content.background_color {
        create_slide_header_with_bg(bg_color)
    } else {
        SLIDE_HEADER.to_string()
    };
    let mut xml = header;
