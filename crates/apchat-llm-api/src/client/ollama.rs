...
/// Ollama server LLM client implementation with OpenAI-compatible API
...
impl LlmClient for OllamaClient {
    ...
    async fn chat_streaming(&self, messages: Vec<ChatMessage>, tools: Vec<ToolDefinition>) -> Result<Box<dyn Stream<Item = Result<StreamingChunk>> + Send + Unpin>> {
        ...
        if !response.status().is_success() {
            let error_text = response.text().await?;
            return Err(anyhow::anyhow!("Ollama streaming API error: {}", error_text));
        }
        ...
        let stream = stream! {
            ...
            print_heart_yellow(&format!("🔍 [Ollama Streaming] Starting stream at {:?}", start_time), true);
            ...
            while let Some(chunk_result) = byte_stream.next().await {
                match chunk_result {
                    Ok(chunk) => {
                        ...
                        print_heart_yellow(&format!("🔍 [Ollama Streaming] Chunk #{}: {} bytes [total: {:?}, since last: {:?}]",
                        ...
                        let chunk_str = match String::from_utf8(chunk.to_vec()) {
                            Ok(s) => s,
                            Err(e) => {
                                print_heart_yellow(&format!("❌ [Ollama Streaming] Invalid UTF-8 in stream: {}", e), true);
                        ...
                        while let Some(newline_pos) = buffer.find('\n') {
                            ...
                            match Self::parse_openai_sse_line(&line) {
                                ...
                                Err(e) => {
                                    print_heart_yellow(&format!("❌ [Ollama Streaming] Parse error: {}", e), true);
                                    yield Err(e);
                    ...
                    Err(e) => {
                        print_heart_yellow(&format!("❌ [Ollama Streaming] Stream error: {}", e), true);
            ...
            if !buffer.is_empty() && !buffer.trim().is_empty() {
                match Self::parse_openai_sse_line(&buffer) {
                    ...
                    Ok(None) => {},
                    Err(e) => {
                        print_heart_yellow(&format!("❌ [Ollama Streaming] Final parse error: {}", e), true);
                        yield Err(e);
                    }
                }
            }
...
impl OllamaClient {
    ...
    async fn build_chat_request(&self, messages: Vec<ChatMessage>, tools: Vec<ToolDefinition>) -> Result<serde_json::Value> {
        let chat_messages: Vec<apchat_models::Message> = messages.into_iter().map(|msg| {
            // Ollama supports system messages, so we can keep them as is
