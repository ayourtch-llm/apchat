# Qwen3-VL Image Encoding Specification

## Executive Summary

This document specifies how APChat will ingest and process images for Qwen3-VL (Qwen3.5 multimodal) models. The specification is based on analysis of the Qwen3-VL architecture, transformers library implementation, and related documentation.

**Key Insight**: Qwen3-VL uses a **dynamic resolution** approach where images are converted into a variable number of visual tokens based on their original resolution, rather than fixed-size resizing.
...
### 1.1 Vision Encoder
- **Model**: Enhanced Vision Transformer (ViT) with DeepStack integration
- **Architecture**: Multi-level ViT features for visual detail capture
- **Integration**: Vision encoder outputs are projected to match language model hidden dimension via an MLP projector
...
# For Qwen2.5-VL (previous version):
...
- Dimensions are rounded to nearest multiple of 32 (Qwen3-VL) or 28 (Qwen2.5-VL)
- Formula:
  ```
...
|-----------|---------------|-------------|-------|
| `patch_size` | 14 | Spatial patch size for ViT | 14, 28 |
| `temporal_patch_size` | 2 | Temporal patch size for video | 2, 4 |
| `max_pixels` | 250880 | Maximum pixels per image | 4096 - 1280*patch_size² |
| `min_pixels` | 4096 | Minimum pixels per image | 4096 - max_pixels |
| `dynamic_patch_size` | True | Enable dynamic resolution | True/False |
| `total_pixels` | N/A | Video token budget | < 24576 * 32 * 32 |
...
| `image_bound` | 151654 | `<|image_pad|>` | Padding token (optional) |
| `vision_start` | 151655 | `<|vision_start|>` | Vision segment start |
| `vision_end` | 151656 | `<|vision_end|>` | Vision segment end |
...
#### Single Image
```
<|vision_start|><|image_start|>
[VISUAL_TOKENS]
<|image_end|><|vision_end|>
[USER TEXT PROMPT]
```
...
### 4.1 Overview
Qwen3-VL uses **Interleaved Multimodal RoPE (MRoPE)** for spatial-temporal modeling:
...
def process_image_for_qwen3vl(image_path: str, max_pixels: int = 250880) -> dict:
    """
    Process image for Qwen3-VL model.
    ...
    # Round to nearest multiple of 32 (Qwen3-VL)
...
def format_qwen3vl_message(images: list[str], text: str) -> dict:
    """
    Format message for Qwen3-VL inference.
...
- [ ] Integration tests with Qwen3-VL models
...
2. **Qwen3-VL GitHub**: https://github.com/QwenLM/Qwen3-VL
3. **Transformers Documentation**: https://huggingface.co/docs/transformers/main/model_doc/qwen3_vl
4. **vLLM Qwen3-VL Support**: https://docs.vllm.ai/en/latest/model_executor/models/qwen3_vl/
5. **Alibaba Cloud Model Studio**: https://www.alibabacloud.com/help/en/model-studio/vision
