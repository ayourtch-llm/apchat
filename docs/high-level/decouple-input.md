...
the same criterion. If needed, a bogus message can be inserted with the text "== interrupted ==" or something along these lines. All the rest of the "/"-commands
should continue working as they do now. Also: pay attention, since the input is now possible at any time, there now needs to be a way to ensure the input is not clobbered, and also to take in the confirmation prompts (somehow from input ?) IMPORTANT: There were already a couple of implementation attempts, the codebase
has a lot of plans and code changes. You may want to study that and reuse or clean up, rather than starting from scratch.
